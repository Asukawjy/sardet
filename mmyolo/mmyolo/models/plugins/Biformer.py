...
class BiLevelRoutingAttention(nn.Module):
    ...
    def __init__(self, in_channels, n_win=7, num_heads=8, qk_dim=None, qk_scale=None,
        ...
        self.qk_dim = qk_dim or in_channels
        assert self.qk_dim % num_heads == 0 and self.dim % num_heads == 0, 'qk_dim and dim must be divisible by num_heads!'
        self.scale = qk_scale or self.qk_dim ** -0.5
 
        ################side_dwconv (i.e. LCE in ShuntedTransformer)###########
        self.lepe = nn.Conv2d(in_channels, in_channels, kernel_size=side_dwconv, stride=1, padding=side_dwconv // 2,
                              groups=in_channels) if side_dwconv > 0 else \
...
if __name__ == '__main__':
    block = BiLevelRoutingAttention(64).cuda()
